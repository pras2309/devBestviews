...
<section class="search-filter">
	<div class="container">
		...
		<?php
		global $wpdb;
		$category = $_GET['review-category'];
		
		$product_list = $wpdb->get_results("SELECT * FROM pgs_products WHERE product_category LIKE '%$category%' ORDER BY score DESC LIMIT 10 ");
		
		$i = 1;
		if(!empty($product_list)):
		?>
			<div class="row">
				<div class="col-sm-12 col-xs-12 col-lg-12 product_body">
					<table class="table table-bordered table-striped table-condensed product_table">
			 <thead>
				<tr>
					<th class="product_table_heading">Rank</th>
					<th class="product_table_heading"></th>
					<th class="product_table_heading">Product Name</th>
					<th class="product_table_heading">Score</th>
					<th class="product_table_heading"></th>	
				</tr> 
			</thead>
			<tbody>
			<?php foreach( $product_list  as $product ): ?>
				<tr class="product_data">
					<td><?php echo $i ; ?> </td>
					<td><img src="<?php echo $product->product_image_url; ?>" alt="image not found" class="img img-responsive" width="120" height="120"/></td>
					<td><?php echo $product->product_name; ?></td>
					<td><?php echo $product->score; ?>/10</td>
					<td><a class="btn btn-primary product_button" href="<?php echo  get_permalink($product->post_id);?>"> View Product</a></td>
				</tr>
			<?php 
					$i++;
				endforeach;
