<div class="gp-post-format-video-content gp-image-above">
	...
	<?php if ( get_post_meta( get_the_ID(), 'video_embed_url', true ) ) { ?>
		...
		<?php global $wp_embed; ?>
		<?php echo $wp_embed->run_shortcode( '[embed width="' . $GLOBALS['ghostpool_image_width'] . '" height="' . $GLOBALS['ghostpool_image_height'] . '"]' . esc_url( get_post_meta( get_the_ID(), 'video_embed_url', true ) ) . '[/embed]' ); ?>
