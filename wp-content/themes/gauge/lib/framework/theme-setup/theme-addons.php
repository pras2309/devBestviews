...
class GhostPool_Addons_Manager {
	...
	function do_plugin_update( $slug ) {
		...
		$status['error'] = esc_html__( 'The plugin does not have an update.', 'gauge' );
		wp_send_json_error( $status );
	...
	function install_child_theme( $slug ) {
		if ( empty( $this->plugins[ $slug ] ) ) {
			wp_send_json_error( array( 'error' => esc_html__( 'We cannot find this child theme.', 'gauge' ) ) );
		}
		...
		if ( ! current_user_can( 'install_themes' ) ) {
			$status['error'] = esc_html__( 'You do not have the permissions to install themes.', 'gauge' );
		...
		if ( isset( $plugin_config['addon_type'] ) && $plugin_config['addon_type'] === 'child_theme' ) {
			...
			if ( $this->is_child_theme_installed() ) {
				...
				if ( $this->is_child_theme_active() ) {
					...
					$status['status_text'] = esc_html__( 'Active', 'gauge' );
					$status['action_text'] = esc_html__( 'Child theme installed and active', 'gauge' );
					$status['action']      = 'no_action';
				} else  {
					$status['status']      = 'gp-inactive';
					$status['status_text'] = esc_html__( 'Inactive', 'gauge' );
					$status['action_text'] = esc_html__( 'Activate child theme', 'gauge' );
					$status['action']      = 'enable_child_theme';
				}
			} else{
				$status['status']      = 'gp-not-installed';
				$status['status_text'] = esc_html__( 'Not installed', 'gauge' );
				$status['action_text'] = esc_html__( 'Install child theme', 'gauge' );
				$status['action']      = 'install_theme';

				if ( ! current_user_can( 'install_themes' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'Permissions needed to install child themes. Contact site administrator.', 'gauge' );
			...
			if ( $this->is_plugin_installed( $plugin_slug ) ) {
				if ( $this->does_plugin_have_update( $plugin_slug ) ) {
					...
					$status['status_text']    = esc_html__( 'Needs update', 'gauge' );
					$status['action_text']    = esc_html__( 'Update plugin', 'gauge' );
					$status['action']         = 'update_plugin';
				}
				elseif ( $this->is_plugin_active( $plugin_slug ) ) {
					$status['status']         = 'gp-active';
					$status['status_text']    = esc_html__( 'Active', 'gauge' );
					$status['action_text']    = esc_html__( 'Deactivate plugin', 'gauge' );
					$status['action']         = 'disable_plugin';
				} else  {
					$status['status']         = 'gp-inactive';
					$status['status_text']    = esc_html__( 'Inactive', 'gauge' );
					$status['action_text']    = esc_html__( 'Activate plugin', 'gauge' );
				...
				$status['status']         = 'gp-not-installed';
				$status['status_text']    = esc_html__( 'Not Installed', 'gauge' );
				$status['action_text']    = esc_html__( 'Install plugin', 'gauge' );
				$status['action']         = 'install_plugin';

				if ( ! current_user_can( 'install_plugins' ) ) {
					$status['status']         = 'gp-not-installed gp-addons-disabled';
					$status['action_text']    = esc_html__( 'You do not have permission to install plugins. Contact the site administrator.', 'gauge' );
					$status['action']         = 'contact_network_admin';
				}
	...
	function do_plugin_install( $slug, $echo = true ) {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$status['error'] = esc_html__( 'You do not have permission to install plugins.', 'gauge' );
			if ( $echo ) {
				wp_send_json_error( $status );
			} else {
				return $status;
	...
	function do_plugin_deactivate( $slug ) {
		...
		if ( empty( $this->plugins[ $slug ] ) ) {
			$status['error'] = esc_html__( 'We have no data about this plugin.', 'gauge' );
			wp_send_json_error( $status );
		}
