	...
	class GhostPool_Custom_Sidebars {
		...
		function deleteSidebar() {
			if ( ! current_user_can( $this->cap_required ) )
				return new WP_Error( 'ghostpool_cant_delete', esc_html__( 'You do not have permission to delete sidebars.', 'gauge' ) );
			...
			if ( $deleted )
				$this->setMessage( sprintf( esc_html__( 'The sidebar "%s" has been deleted.', 'gauge' ), $_REQUEST['delete'] ) );
			else
				$this->setError( sprintf( esc_html__( 'There was not any sidebar called "%s" and it could not been deleted.', 'gauge' ), $_GET['delete'] ) );
		}
		...
		function addSubMenus() {
			$page = add_theme_page( esc_html__( 'Sidebars', 'gauge' ), esc_html__( 'Sidebars', 'gauge' ), $this->cap_required, 'sidebars', array( $this, 'createPage' ) );
		...
		function updateModifiable() {
			...
			$this->setMessage( esc_html__( 'The custom sidebars settings has been updated successfully.', 'gauge' ) );
		}
		...
		function storeSidebar() {
			...
			if ( empty( $name ) OR empty( $description ) )
				$this->setError( esc_html__( 'You have to fill all the fields to create a new sidebar.', 'gauge' ) );
			else{
				...
				if ( $sidebars !== FALSE ) {
					...
					if ( ! $this->getSidebar( $id,$sidebars )  ) {
						...
						$this->setMessage( esc_html__( 'The sidebar has been created successfully.', 'gauge' ) );
					
					
					}
					else
						$this->setError( esc_html__( 'There is already a sidebar registered with that name, please choose a different one.', 'gauge' ) );
				}
				else{
					$id = $this->sidebar_prefix . sanitize_html_class( sanitize_title_with_dashes( $name ) );
		...
		function updateSidebar() {
			...
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {} else {
						if ( isset( $url['query'] ) ) {
								...
								if ( $args['id'] != $id )
										return new WP_Error( esc_html__( 'The operation is not secure and it cannot be completed.', 'gauge' ) );
						}
						else
								return new WP_Error( esc_html__( 'The operation is not secure and it cannot be completed.', 'gauge' ) );
					}
			...
			$this->setMessage( sprintf( esc_html__( 'The sidebar "%s" has been updated successfully.', 'gauge' ), $id ) );
		}
		...
		function getEmptyWidget() {
			return array( 
				'name' => esc_html__( 'GhostPool Empty Widget', 'gauge' ),
				'id' => 'gp-empty-widget',
				...
				'description' => esc_html__( 'Custom sidebar description', 'gauge' ),
			 );
		}
		...
		function resetSidebars() {
			...
			$this->setMessage( esc_html__( 'The custom sidebars data has been removed successfully.', 'gauge' ) );	
		}
