...
if ( ! class_exists( 'Ghostpool_Custom_Menu' ) ) {
	...
	class Ghostpool_Custom_Menu extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			// Fallbacks Gauge v3.0+
			if ( preg_match( '/megamenu/', $class_names ) ) {
				$menu_type = get_post_meta( $item->ID, 'menu-item-menu-type', true ) ? get_post_meta( $item->ID, 'menu-item-menu-type', true ) : 'megamenu';
			} else {
				$menu_type = get_post_meta( $item->ID, 'menu-item-menu-type', true ) ? get_post_meta( $item->ID, 'menu-item-menu-type', true ) : 'standard-menu';
			}
			
			// Profile class
			if ( $menu_type == 'gp-profile-link' ) {
				$profile_class = 'standard-menu';
			} else {
				$profile_class = '';
			}		
			...
			if ( ( is_user_logged_in() && get_post_meta( $item->ID, 'menu-item-gp-user-display', true ) != 'gp-show-logged-out' ) OR ( ! is_user_logged_in() && get_post_meta( $item->ID, 'menu-item-gp-user-display', true ) != 'gp-show-logged-in' ) ) {
				...
				if ( ( is_user_logged_in() && ( $menu_type == 'gp-login-link' OR $menu_type == 'gp-register-link' ) ) ) {
					...
					$output .= $indent . '<li id="nav-menu-item-'. $item->ID . '" class="' . $menu_type . ' ' . $profile_class . ' ' . get_post_meta( $item->ID, 'menu-item-columns', true ) . ' ' . get_post_meta( $item->ID, 'menu-item-content', true ) . ' ' . get_post_meta( $item->ID, 'menu-item-gp-display', true ) . ' ' . $depth_class_names . ' ' . get_post_meta( $item->ID, 'menu-item-gp-hide-nav-label', true ) . ' ' . $class_names . '">';
					...
					// Tab content menu
					if ( $menu_type == 'tab-content-menu' OR $menu_type == 'content-menu' ) {
								...
								if ( $menu_type == 'tab-content-menu' ) {
									...
									if ( ! empty( $terms ) ) {
										$dropdown .= '<ul class="gp-menu-tabs">
											<li id="' . $item->object_id . '" class="gp-selected">' . esc_html__( 'All', 'gauge' ) . '</li>';		
								...
								while ( $query->have_posts() ) : $query->the_post();
									...
									$dropdown .= '<section class="' . implode( ' ' , get_post_class( 'gp-post-item' ) ) . '">';
										...
										if ( has_post_thumbnail() ) {
											...
											$image = aq_resize( wp_get_attachment_url( get_post_thumbnail_id( get_the_ID() ) ), apply_filters( 'gp_menu_image_width', '198' ), apply_filters( 'gp_menu_image_height', '125' ), true, false, true );
											if ( ghostpool_option( 'retina', '', 'gp-retina' ) == 'gp-retina' ) {
												$retina = aq_resize( wp_get_attachment_url( get_post_thumbnail_id( get_the_ID() ) ), apply_filters( 'gp_menu_image_width', '198' ) * 2, apply_filters( 'gp_menu_image_height', '125' ) * 2, true, true, true );
											} else {
												$retina = '';
											}
									
											$dropdown .= '<div class="gp-post-thumbnail"><div class="gp-image-above">
												<a href="' . $link . '" title="' . ghostpool_prefix_hub_title( get_the_ID() ) . '"' . $target . '>
													<img src="' . $image[0] . '" data-rel="' . $retina . '" width="' . $image[1] . '" height="' . $image[2] . '" alt="' . ghostpool_prefix_hub_title( get_the_ID() ) . '" class="gp-post-image" />
												</a>
											</div></div>';
							
										}
								
										$dropdown .= '<h3 class="gp-loop-title"><a href="' . $link . '" title="' . ghostpool_prefix_hub_title( get_the_ID() ) . '"' . $target . '>' . ghostpool_prefix_hub_title( get_the_ID() ) . '</a></h3>
						
									</section>';
						
								endwhile; 
					
								if ( ghostpool_option( 'ajax' ) == 'ajax-loop' ) {
									$dropdown .= '</div><div class="gp-pagination gp-standard-pagination gp-pagination-arrows">' . ghostpool_get_previous_posts_page_link( $query->max_num_pages ) . ghostpool_get_next_posts_page_link( $query->max_num_pages ) . '</div>';
								}
