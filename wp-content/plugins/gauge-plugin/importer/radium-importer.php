	...
	class Radium_Theme_Importer {
    	...
    	public function intro_html() {
			...
			<div class="gp-primary-text">
				...
				<p><?php esc_html_e( 'Importing demo data is the easiest way to setup your theme. It will allow you to quickly edit everything seen in the theme demo instead of creating everything from scratch. When you import the demo data please note the following:', 'gauge-plugin' ); ?></p>

				<ul class="ul-square">
					<li><?php esc_html_e( 'No existing data will be deleted or modified.', 'gauge-plugin' ); ?></li>
					<li><?php esc_html_e( 'Posts, pages, categories, media, widgets and menus will be imported.', 'gauge-plugin' ); ?></li>
					<li><?php wp_kses( _e( 'To import WooCommerce and bbPress data please activate these plugins <strong>before</strong> importing the demo data.', 'gauge-plugin' ), array( 'strong' => array() ) ); ?>
					<li><?php wp_kses( _e( 'BuddyPress Activity, Groups and Members seen in the demo will NOT be imported. You can import dummy BuddyPress data using the <a href="https://wordpress.org/plugins/bp-default-data/" target="_blank">BuddyPress Default Data</a> plugin.', 'gauge-plugin' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?>
				</ul>
	    ...
	    public function demo_installer() {
			...
			<div class="gp-theme-setup postbox">
				...
				<h3 class="gp-primary-header"><?php esc_html_e( 'Import Demo Data' , 'gauge-plugin' ); ?></h3>
				...
				<div class="radium-importer-wrap gp-primary-text inside" data-demo-id="1" data-nonce="<?php echo wp_create_nonce( 'radium-demo-code' ); ?>">
					...
					<form method="post" class="js-one-click-import-form">
						...
						<input name="reset" class="panel-save radium-import-start gp-support-button button button-primary" type="submit" value="<?php esc_attr_e( 'Import Demo Data', 'gauge-plugin' ); ?>" />
						...
						<script>
							jQuery( function ( $ ) {
								$( '.js-one-click-import-form' ).on( 'submit', function () {
									...
									$( this ).append( '<p><span class="spinner"></span><?php esc_html_e( "Importing now, this can take a few minutes", "gauge-plugin" ); ?></p>' );
						...
						<?php if ( 'demo-data' == $action && check_admin_referer( 'radium-demo-code' , 'demononce' ) ) { ?>
							...
							<?php if ( $this->flag_as_imported['content'] && $this->flag_as_imported['widgets'] ) { ?>
								<div class="radium-importer-success">
									<p><?php esc_html_e( 'You have successfully imported the demo data.', 'gauge-plugin' ); ?></p>
								</div>
							<?php } ?>
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'gauge-plugin' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'gauge-plugin' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'gauge-plugin' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'gauge-plugin' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'gauge-plugin' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'gauge-plugin' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'gauge-plugin' );
	    				}
	    			...
	    			//$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'gauge-plugin' ); // show "No Title" if widget instance is untitled
