...
function sc_social_connect_process_login( $is_ajax = false ) {
	...
	switch( $social_connect_provider ) {
		case 'facebook':
			...
			$fb_json = json_decode( sc_http_get_contents("https://graph.facebook.com/me?access_token=" . $_REQUEST['social_connect_access_token'].'&fields=id,name,email,first_name,last_name') );
...
function sc_filter_avatar($avatar, $id_or_email, $size, $default, $alt) {
	...
	$social_id = '';
	$provider_id = '';
	$user_id = (!is_integer($id_or_email) && !is_string($id_or_email) && is_object($id_or_email)) ? $id_or_email->user_id : $id_or_email;
